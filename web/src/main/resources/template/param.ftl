...
<#-- 逻辑删除注解 -->
    ...
    <#if field.propertyType == 'LocalDateTime'  || field.propertyType == 'LocalDate'>
        ...
        @ApiModelProperty(value = "${field.comment} 开始时间")
        @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss")
        private Date ${field.propertyName}StartTime;

        @ApiModelProperty(value = "${field.comment} 结束时间")
        @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss")
        private Date ${field.propertyName}EndTime;
    <#else>
